  ...
  return (
    <main className="flex flex-col gap-5 px-4 pb-6 pt-6 md:mx-auto md:max-w-2xl md:px-8 md:pt-8">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-base font-bold text-brand-700">Edit Profil</h1>
      </div>

      {isLoading && <p className="text-center text-sm text-gray-400">Memuat profil...</p>}
      {error && <p className="text-center text-sm text-red-500">{error}</p>}
      {profile && <EditProfileForm initialProfile={profile} />}
    </main>
  );
}
